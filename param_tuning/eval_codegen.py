...
from util import check_vllm_stderr, compose_codegen_cmd
...
def main(args: argparse.Namespace):
    ...
    if args.sampling_n == 1:
        assert args.temperature == 0
    else:
        assert args.temperature > 0
    
    assert args.dataset in ['humaneval', 'mbpp_plus']
    
    if 1000 * args.kv_quant_thresh == int(1000 * args.kv_quant_thresh):
        args.log_path = (f'{args.log_path}/{args.dataset}_k{args.sampling_n}/'
    ...
    if num_samples == 0:
        ...
        if args.dataset == 'humaneval':
            num_samples = len(HumanEvalDataset().dataset[label])
        else:
            num_samples = len(MbppPlusDataset().dataset[label])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sampling-n', type=int, default=1)
