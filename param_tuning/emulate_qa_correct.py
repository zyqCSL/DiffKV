...
def main(args: argparse.Namespace):
    ...
    assert args.dataset in ['gsm8k', 'mmlu_cot']
    ...
    if num_samples == 0:
        ...
        if args.dataset == 'gsm8k':
            num_samples = len(GSM8kDataset().dataset[label])
        elif args.dataset == 'mmlu_cot':
            num_samples = len(MMLUCoTDataset().dataset[label])
        else:
    ...
    assert attn_prune_thresh >= 0 and attn_prune_thresh <= 1, attn_prune_thresh
...
# # dataset config
# LABEL = 'train'
# TOTAL_SAMPLES = len(GSM8kDataset().dataset[LABEL])
# PROMPT_LIMIT = 3500
