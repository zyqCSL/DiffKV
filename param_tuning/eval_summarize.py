...
def eval_summarize(
    ...
    for i, p in enumerate(processes):
        ...
        cum_rogues.append(
           (partition_metric_df['rogue-1'][0] + \
            partition_metric_df['rogue-2'][0] + \
            partition_metric_df['rogue-l'][0]) * num_seqs)
        
        cum_rogue_1.append(
           partition_metric_df['rogue-1'][0] * num_seqs)
        cum_rogue_2.append(
           partition_metric_df['rogue-2'][0] * num_seqs)
        cum_rogue_l.append(
           partition_metric_df['rogue-l'][0] * num_seqs)
    ...
    unified_rogue = np.sum(cum_rogues) / np.sum(all_num_seqs) * 1000
    unified_rogue_1 = np.sum(cum_rogue_1) / np.sum(all_num_seqs)
    unified_rogue_2 = np.sum(cum_rogue_2) / np.sum(all_num_seqs)
    unified_rogue_l = np.sum(cum_rogue_l) / np.sum(all_num_seqs)
    ...
    noise_free_eval = {
        'rogue': (unified_rogue, 0), 
        'rogue-1': (unified_rogue_1, 0),
        'rogue-2': (unified_rogue_2, 0),
        'rogue-l': (unified_rogue_l, 0),
...
def main(args: argparse.Namespace):
    ...
    if num_samples == 0:
        # dataset config
        if args.dataset == 'cnn_daily':
            num_samples = len(CNNDailyMailDataset().dataset[label])
        else:
