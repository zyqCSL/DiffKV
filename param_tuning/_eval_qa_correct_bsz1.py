...
GPUS = list(range(0, 4))
GPU_MEM_UTIL = 0.97
BATCH_SIZE = 1 # max number of concurrently running seqs in vllm
# MAX_PADDING = 4096
...
def main(args: argparse.Namespace):
    ...
    assert args.dataset in ['gsm8k', 'minerva_math', 'mmlu_cot', 'mmlu_pro_cot', 'gpqa', 'aime']
    ...
    if sample_rate is not None:
        assert args.dataset in ['minerva_math', 'aime']
    ...
    if num_samples == 0:
        ...
        if args.dataset == 'gsm8k':
            ...
            if not _sample_rate:
                _sample_rate = 100
            num_samples = len(MinervaMathDataset(
                label=label,
                sample_percent=_sample_rate,
            ).dataset)
        elif args.dataset == 'mmlu_cot':
            num_samples = len(MMLUCoTDataset().dataset[label])
        elif args.dataset == 'mmlu_pro_cot':
            num_samples = len(MMLUProCoTDataset().dataset[label])
        elif args.dataset == 'gpqa':
            num_samples = len(GPQACoTDataset().dataset)
        elif args.dataset == 'aime':
            num_samples = len(AIMEDataset().dataset)
        else:
