...
template<>
inline __device__ Float4_ mul(Float4_ a, Float4_ b) {
  Float4_ c;
  c.x = mul<float2, float2, float2>(a.x, b.x);
  c.y = mul<float2, float2, float2>(a.y, b.y);
  return c;
}
...
inline __device__ Float4_ fma(Float4_ a, Float4_ b, Float4_ c) {
  Float4_ d;
  d.x = fma(a.x, b.x, c.x);
  d.y = fma(a.y, b.y, c.y);
  return d;
...
inline __device__ Float16_ to_float(Float16_ u) {
  return u;
}
