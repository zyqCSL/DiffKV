...
inline __device__ float4 half4_to_float4(uint64_t v) {
  uint32_t lo, hi;
  asm volatile("mov.b64 {%0, %1}, %2;\n" : "=r"(lo), "=r"(hi) : "l"(v));
  float2 lo_float2 = half2_to_float2(lo);
  float2 hi_float2 = half2_to_float2(hi);
  return make_float4(lo_float2.x, lo_float2.y, hi_float2.x, hi_float2.y);
}
...
inline __device__ Float16_ to_float(ulong4 u) {
  Float16_ tmp;
  tmp.x = half4_to_float4(u.x);
  tmp.y = half4_to_float4(u.y);
  tmp.z = half4_to_float4(u.z);
  tmp.w = half4_to_float4(u.w);
  return tmp;
}
