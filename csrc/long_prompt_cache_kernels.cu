...
__global__ void compress_and_append_cache_long_prompt_phase_kernel(
  ...
  const float* __restrict__ score,                   // unsorted score: [num_seqs, num_kv_heads, max_prompt_len]
  ...
  const int max_num_blocks_per_seq)
{
  ...
  for (int token_idx = thread_idx + count_pruned; token_idx < count_pruned + count_quantized; token_idx += NUM_THREADS_PROMPT) {
    ...
    const int64_t src_idx_base = (prompt_start + position) * kv_stride
...
#define LAUNCH_CACHE_KERNEL_LONG_PROMPT_PHASE(T, HEAD_SIZE, BITS_K_HIGH, BITS_V_HIGH,             \
                                              BITS_K_LOW, BITS_V_LOW,                             \
                                              NUM_TOKENS_PER_PAGE_HIGH, NUM_TOKENS_PER_PAGE_LOW,  \
                                              THREAD_GROUP_SIZE_V)                                \
  vllm::compress_and_append_cache_long_prompt_phase_kernel<T, HEAD_SIZE, BITS_K_HIGH, BITS_V_HIGH,             \
                                                           BITS_K_LOW, BITS_V_LOW,                             \
                                                           NUM_TOKENS_PER_PAGE_HIGH, NUM_TOKENS_PER_PAGE_LOW,  \
                                                           THREAD_GROUP_SIZE_V>                                \
...
void compress_and_append_cache_long_prompt_phase(
  ...
  torch::Tensor& score,                  // [num_seqs, num_kv_heads, max_prompt_len]
