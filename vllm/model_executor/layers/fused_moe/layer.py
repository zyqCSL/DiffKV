...
class UnquantizedFusedMoEMethod(FusedMoEMethodBase):
    ...
    def create_weights(self, num_experts: int,
                       hidden_size: int, intermediate_size_per_partition: int,
                       params_dtype: torch.dtype,
                       extra_weight_attrs: Dict[str, Any]) -> Dict[str, torch.Tensor]:
...
class FusedMoE(torch.nn.Module):
    ...
    def __init__(
        ...
        # NOTE: we don't support dp > 1 for now
        self.dp_size = 1
        # rank within the dp group
        self.dp_rank = 0
        self.global_num_experts = num_experts

        # Use expert parallelism instead of tensor parallelism?
        use_ep = (enable_expert_parallel
        ...
        self.fused_moe_weights = self.quant_method.create_weights(
            num_experts=self.local_num_experts,
            hidden_size=hidden_size,
            intermediate_size_per_partition=self.intermediate_size_per_partition,
            params_dtype=params_dtype,
            extra_weight_attrs=moe_quant_params,
        )
        for name, weight in self.fused_moe_weights.items():
    ...
    def weight_loader(self, param: torch.nn.Parameter,
        ...
        # NOTE: Not supporting GGUF for now
    ...
    def forward_impl(self, hidden_states: torch.Tensor,
        ...
        # Matrix multiply.
        final_hidden_states = self.quant_method.apply_weights(
            weights=self.fused_moe_weights,
            x=hidden_states,
            router_logits=router_logits,
            top_k=self.top_k,
